...
This plugin allows you to use OpenAI for code review in Gerrit conveniently. After submitting a Patch Set, OpenAI will
provide review feedback in the form of comments and, optionally, a vote.
You can continue to ask OpenAI by @{gerritUserName} or @{gerritEmailAddress} (provided that `gerritEmailAddress` is in
the form "gerritUserName@<any_email_domain>") in the comments to further guide it in generating more targeted review
comments.
Reviews can be also triggered by directing a comment with the `/review` command to OpenAI.
...
3. **Configure:** First, you need to create an OpenAI user in Gerrit.
...
- `gerritUserName`: Gerrit username of OpenAI user.
...
### OpenAI Score Adjustment Following User Interaction

In the example below, OpenAI initially posits a potential unintended behavior in the code, assigning a "-1" score.
Upon receiving clarification, it resets the score to "0".

![Example of Dialogue](images/chatgpt_changed_mind.png?raw=true)

More examples of OpenAI's code reviews and inline discussions are available at
...
OpenAI Backend uses the **Assistant** resource to maintain a richer interaction context. This mode is designed to:
...
- `backendAI`: Select the mode the requests are processed. The default value is `OPENAI`.
- `gptModel`: The default model is `gpt-4o`. You can also configure it to `gpt-3.5-turbo` or `gpt-4-turbo`.
    - `gptSystemPromptInstructions`: You can customize the default instructions ("Act as a PatchSet Reviewer") to your
      preferred prompt.
- `gptReviewTemperature`: Specifies the temperature setting for OpenAI when reviewing a Patch Set, with a default
...
- `gptCommentTemperature`: Specifies the temperature setting for OpenAI when replying to a comment, with a default
  setting of 1.0.
...
- `enabledTopicFilter`: Specifies a list of keywords that trigger OpenAI reviews based on the topic of the Patch Set.
...
- `directive`: Directives are mandatory instructions written in plain English that OpenAI must adhere to during its
...
- `enabledVoting`: Initially disabled (false). If set to true, allows OpenAI to cast a vote on each reviewed Patch Set
...
- `filterRelevantComments`: This setting is enabled by default (true) to display only those review comments considered
  relevant by OpenAI, which means they have a relevance index at or above the `filterCommentsRelevanceThreshold`
  specified below. Turning off this option (false) allows the display of comments OpenAI marks as irrelevant.
- `filterCommentsRelevanceThreshold`: When `filterRelevantComments` is enabled, any review comment assigned a relevance
  score bytOpenAI below this threshold will not be shown. The default threshold is set at 0.6.
- `gptRelevanceRules`: This option allows customization of the rules OpenAI uses to determine the relevance of a task.
- `patchSetCommentsAsResolved`: Initially set to false, this option leaves OpenAI's Patch Set comments as unresolved,
  inviting further discussion. If activated, it marks OpenAI's Patch Set comments as resolved.
- `inlineCommentsAsResolved`: Initially set to false, this option leaves OpenAI's inline comments as unresolved,
  inviting further discussion. If activated, it marks OpenAI's inline comments as resolved.
...
- `gptFullFileReview`: Enabled by default. Activating this option sends both unchanged lines and changes to OpenAI for
  review, offering additional context information. Deactivating it (set to false) results in only the changed lines
  being submitted for review.
- `ignoreResolvedOpenAIComments`: Determines if resolved comments from OpenAI should be disregarded. The default
  setting is true, which means resolved OpenAI comments are not used for generating new comments or identifying
  duplicate content. If set to false, resolved OpenAI comments are factored into these processes.
...
- `codeContextPolicy`: Defines the code context policy to provide OpenAI with the missing code context from the
  ChangeSet. The currently supported policies are:
    - **ON_DEMAND**: Fetches just the minimal set of code artifacts requested by the Model. These artifacts can be
      ...
      the change.
    - **UPLOAD_ALL**: Uploads the entire codebase during each merge event, giving OpenAI full access to the necessary
...
- `taskSpecificAssistants`: This option allows for dividing the Patch Set review between two specialized assistants: one
  ...
  **NOTE**: Enabling this feature may result in duplicate requests to OpenAI, potentially increasing the usage costs of
  the OpenAI API.
...
To send messages to OpenAI, simply mention the OpenAI user in plain English within the message. However, if the
...
message to OpenAI, rather than as a command.
...
ensures the message is correctly forwarded to OpenAI.
...
- `/review`: when used in a comment directed at OpenAI on any Change Set, triggers a review of the full Change Set. A
  vote is cast on the Change Set if the voting feature is enabled and the OpenAI Gerrit user is authorized to vote on
  it.
- `/review_last`: when used in a comment directed at OpenAI on any Change Set, triggers a review of the last Patch Set
...
- `--debug`: When paired with `/review` or `/review_last` commands, this option displays useful debug information in
  each OpenAI reply, showing all replies as though the filter setting were disabled.
...
You can now dynamically alter the plugin configuration via messages sent to the OpenAI user, primarily for testing and
...
Directives are mandatory instructions written in plain English that OpenAI must adhere to during its reviews. In
...
Threads capture all prior interactions and evaluations involving OpenAI within each Change Set. The history stored in
these threads can be removed with the `/forget_thread` command. This functionality is crucial for preventing OpenAI
...
ignoreResolvedOpenAIComments: true
...
This command allows you to force the upload of the Git project's codebase to OpenAI.
...
To filter DEBUG messages containing the OpenAI request and response bodies, the pipe ("|") prefix must be used:
...
Once `enableMessageDebugging` is enabled, you can obtain additional useful debug information in each OpenAI reply, such
...
```
@gpt /configure --backendAI=OPENAI --gptModel=gpt-4-turbo
```
