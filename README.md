...
- `aiDomain`: Specifies the base domain for the OpenAI-compatible API. Defaults match the selected backend:
  `https://api.openai.com` for OpenAI, `https://api.openai.com` for LangChain `OPENAI`,
  `https://generativelanguage.googleapis.com` for `GEMINI`, and `https://api.moonshot.ai` for `MOONSHOT`. Override only
  when you need a custom endpoint; leaving it unset lets the plugin pick the provider default automatically.
