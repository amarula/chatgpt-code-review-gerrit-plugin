...
- `aiDomain`: Specifies the base domain for the OpenAI-compatible API. Defaults match the selected backend:
  `https://api.openai.com` for OpenAI, `https://generativelanguage.googleapis.com` for Gemini, and
  `https://api.moonshot.ai` for Moonshot. Override only when you need a custom endpoint; leaving it unset lets the
  plugin pick the provider default automatically.
