...
import com.googlesource.gerrit.plugins.chatgpt.model.api.gerrit.GerritFileDiff;
...
@Slf4j
public class GerritClientPatchSetImpl extends GerritClientAccount {
  ...
  public GerritClientPatchSetImpl(Configuration config, AccountCache accountCache) {
  ...
  private void processFileDiff(String filename, DiffInfo diff) {
    ...
    Optional.ofNullable(diff.metaA)
        .ifPresent(meta -> gerritPatchSetFileDiff.setMetaA(GerritClientPatchSetImpl.toMeta(meta)));
    Optional.ofNullable(diff.metaB)
        .ifPresent(meta -> gerritPatchSetFileDiff.setMetaB(GerritClientPatchSetImpl.toMeta(meta)));
    Optional.ofNullable(diff.content)
        .ifPresent(
            content ->
                gerritPatchSetFileDiff.setContent(
                    content.stream().map(GerritClientPatchSetImpl::toContent).collect(toList())));
