...
@Slf4j
public class ChatGptPromptRequests extends ChatGptPromptBase
    implements IChatGptPrompt {
  public static String DEFAULT_GPT_ASSISTANT_INSTRUCTIONS_REQUESTS;

  public ChatGptPromptRequests(
      Configuration config,
      ChangeSetData changeSetData,
      GerritChange change,
      ICodeContextPolicy codeContextPolicy) {
    super(config, changeSetData, change, codeContextPolicy);
    loadDefaultPrompts("promptsRequests");
    log.debug(
        "ChatGptPromptRequests initialized for change ID: {}", change.getFullChangeId());
  }
  ...
  private String getCommentRequestPrompt(int commentPropertiesSize) {
    log.debug(
        "Constructing comment request prompt for {} comment properties.",
