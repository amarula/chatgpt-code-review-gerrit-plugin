...
@Slf4j
public abstract class ChatGptClient extends ChatGptClientBase {
  ...
  protected ChatGptResponseContent extractContent(Configuration config, String body)
      throws Exception {
    ...
    if (config.getGptStreamOutput() && !isCommentEvent) {
      ...
      ChatGptResponseUnstreamed chatGptResponseUnstreamed =
          jsonToClass(body, ChatGptResponseUnstreamed.class);
      return getResponseContent(
          chatGptResponseUnstreamed.getChoices().get(0).getMessage().getToolCalls());
    }
  }

  protected boolean validateResponse(
      ChatGptResponseContent chatGptResponseContent, String changeId, int attemptInd) {
