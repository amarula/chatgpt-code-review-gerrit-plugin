...
public class ChatGptThreadMessage extends ChatGptApiBase {
  ...
  private Request createRetrieveMessageRequest(String messageId) {
    String uri = UriResourceLocator.threadMessageRetrieveUri(threadId, messageId);
  ...
  private Request addMessageRequest() {
    String uri = UriResourceLocator.threadMessagesUri(threadId);
    ...
    addMessageRequestBody =
        ChatGptRequestMessage.builder()
            .role("user")
            .content(chatGptPrompt.getDefaultGptThreadReviewMessage(patchSet))
            .build();
