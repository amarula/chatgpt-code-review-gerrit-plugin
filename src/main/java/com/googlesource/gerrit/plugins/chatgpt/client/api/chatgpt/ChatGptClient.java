...
import com.googlesource.gerrit.plugins.chatgpt.client.api.chatgpt.endpoint.ChatGptThread;
import com.googlesource.gerrit.plugins.chatgpt.client.api.chatgpt.endpoint.ChatGptThreadMessage;
import com.googlesource.gerrit.plugins.chatgpt.model.api.chatgpt.ChatGptThreadMessageResponse;
...
@Slf4j
@Singleton
public class ChatGptClient extends AbstractChatGptClient implements IChatGptClient {
  ...
  public ChatGptClient(
    ...
    log.debug("Initialized ChatGptClient.");
  }
  ...
  public ChatGptResponseContent ask(
    ...
    log.info(
        "Processing ChatGPT Request with changeId: {}, Patch Set: {}",
  ...
  private ChatGptResponseContent askSingleRequest(
      ChangeSetData changeSetData, GerritChange change, String patchSet)
      throws OpenAiConnectionFailException {
    log.debug("Processing Single Request");
    String threadId = createThreadWithMessage(changeSetData, change, patchSet);
    runThread(changeSetData, change, threadId);
    ChatGptResponseContent chatGptResponseContent = getChatGptResponseContent(threadId);
  ...
  private String createThreadWithMessage(
    ...
    ChatGptThread chatGptThread =
        new ChatGptThread(config, changeSetData, pluginDataHandlerProvider);
  ...
  private ChatGptResponseContent getChatGptResponseContent(String threadId)
      throws OpenAiConnectionFailException {
    return switch (chatGptRunHandler.getFirstStepDetails().getType()) {
      ...
      default ->
          throw new IllegalStateException(
              "Unexpected Step Type in ChatGpt response: " + chatGptRunHandler);
    };
  }

  private ChatGptResponseContent retrieveThreadMessage(String threadId)
      throws OpenAiConnectionFailException {
    ...
    ChatGptThreadMessageResponse threadMessageResponse =
        chatGptThreadMessage.retrieveMessage(messageId);
