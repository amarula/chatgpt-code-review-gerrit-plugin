...
import com.googlesource.gerrit.plugins.chatgpt.client.code.context.ondemand.CodeContextBuilder;
import com.googlesource.gerrit.plugins.chatgpt.model.api.chatgpt.ChatGptToolOutput;
...
public class ChatGptRunToolOutputHandler extends ChatGptClientBase {
  // ChatGPT may occasionally return the fixed string "multi_tool_use" as the function name when
  // multiple tools are
  // utilized.
  ...
  public void submitToolOutput(List<ChatGptToolCall> chatGptToolCalls)
  ...
  private String getOutput(int i) {
    ...
    if (ON_DEMAND_FUNCTION_NAMES.contains(function.getName())) {
      ChatGptGetContextContent getContextContent =
          getArgumentAsType(chatGptToolCalls, i, ChatGptGetContextContent.class);
