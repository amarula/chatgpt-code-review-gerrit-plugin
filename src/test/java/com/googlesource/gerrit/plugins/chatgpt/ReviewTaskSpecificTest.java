...
public class ReviewTaskSpecificTest extends ChatGptReviewTestBase {
  private static final String CHAT_GPT_REVIEW_ASSISTANT_ID = "asst_TEST_REVIEW_ASSISTANT_ID";
  private static final String CHAT_GPT_COMMIT_MESSAGE_ASSISTANT_ID =
  ...
  protected void setupMockRequests() throws RestApiException {
    ...
    setupMockRequestCreateRun(
        CHAT_GPT_REVIEW_ASSISTANT_ID, CHAT_GPT_REVIEW_RUN_ID, Scenario.STARTED, SECOND_CALL);
    setupMockRequestCreateRun(
        CHAT_GPT_COMMIT_MESSAGE_ASSISTANT_ID, CHAT_GPT_COMMIT_MESSAGE_RUN_ID, SECOND_CALL);
    setupMockRequestRetrieveRunStepsFromBody(
        filterOutSubsetRunStepsResponse(1, 2), CHAT_GPT_REVIEW_RUN_ID);
    setupMockRequestRetrieveRunStepsFromBody(
        filterOutSubsetRunStepsResponse(0, 1), CHAT_GPT_COMMIT_MESSAGE_RUN_ID);
  }

  private String filterOutSubsetRunStepsResponse(int from, int to) {
    ChatGptListResponse runStepsResponse =
        jsonToClass(
            readTestFile(RESOURCE_PATH + "chatGptRunStepsResponse.json"),
  ...
  public void patchSetCreatedOrUpdated() throws Exception {
    ...
    IChatGptPrompt chatGptPromptCommitMessage =
        ChatGptPromptFactory.getChatGptPrompt(config, changeSetData, getGerritChange(), getCodeContextPolicy());
    String reviewPrompt =
        chatGptPromptCommitMessage.getDefaultGptThreadReviewMessage(formattedPatchContent);
