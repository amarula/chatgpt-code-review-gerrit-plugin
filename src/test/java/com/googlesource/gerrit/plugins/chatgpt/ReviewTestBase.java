...
import com.googlesource.gerrit.plugins.chatgpt.client.api.gerrit.GerritClientComments;
import com.googlesource.gerrit.plugins.chatgpt.client.api.gerrit.GerritClientFacade;
...
public class ReviewTestBase extends TestBase {
  ...
  private IChatGptClient getChatGptClient() {
    return config.getGptReviewCommitMessages() && config.getTaskSpecificAssistants()
        ? new ChatGptClientTaskSpecific(config, getCodeContextPolicy(), pluginDataHandlerProvider)
        : new ChatGptClient(config, getCodeContextPolicy(), pluginDataHandlerProvider);
  }

  private IGerritClientPatchSet getGerritClientPatchSet() {
    return new GerritClientPatchSet(config, accountCacheMock);
  }
}
